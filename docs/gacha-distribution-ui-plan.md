...
- **API変更なし**（だんごシェアリンク既存外部 API のみ）
- **ツール主導 / だんごシェアリンク主導** の二フローを UI で明示
- 配布マッピング変更時は `PUT gacha-config` で だんごシェアリンク のレア度表示を追従（プールの現状を送る）
...
- 大量アップロード・大量 claim 再同期で 429 が続く場合のみ だんごシェアリンク バッチ API を検討
