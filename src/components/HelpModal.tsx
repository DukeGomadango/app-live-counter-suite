                ...
                sections: [
                    ...
                    },
                    {
                        ...
                        items: [
                            ...
                            "取り込み時は、バックアップの対象範囲に含まれる安全なキーだけを反映できます",
                            "項目同士を足し合わせるマージはしません",
                        ],
                    },
                    {
                        ...
                        items: [
                            ...
                            "利用には Google Cloud での認証用クライアント設定と、サイト側の環境変数 NEXT_PUBLIC_GOOGLE_CLIENT_ID が必要です",
                        ],
                    },
                    {
                        title: "QR・NFC",
                ...
                sections: [
                    {
                        ...
                        items: [
                            ...
                            "透過PNGの立ち絵を背景にすると、「立ち絵の形で全体を覆う」で輪郭に沿った覆いを自動作成できます（画像タブ）",
                    ...
                    },
                    {
                        ...
                        items: [
                            ...
                            "透過PNGの立ち絵では「立ち絵の形で覆いにする」を押すと、線で区切った領域を立ち絵の不透明部分だけに切り詰めて覆いにできます。線は立ち絵の外から引いても構いません。確定前にキャンバス上でプレビューできます",
                    ...
                    },
                    {
                        title: "保存・共有",
                        items: [
                            "ヘッダー右側（ヘルプの左）の「画像を保存して X で共有」ボタンで共有できます。返信先を設定していない場合、対応端末では共有シートから画像付きで共有できます。返信先を設定している場合は、画像保存後にXの返信画面（in_reply_to付き）を開きます。キャプチャはトリミングした枠に合わせて出力されるため、余白なしで共有できます",
                ...
                sections: [
                    {
                        ...
                        items: [
                            ...
                            "だんごシェアリンクとキャンペーンを連携しているときは、そのキャンペーン用のプレイヤーだけプレイヤータブに表示されます",
                            "連携中にプレイヤー名を変更すると、だんごシェアリンク側の受取人名にも反映されます",
                            "連携中にプレイヤーを削除すると、だんごシェアリンク側ではガチャとの連携だけが解除されます（受付で入場済みの受取枠は残ります）",
                            "同一人物はだんごシェアリンクの受取人名簿で管理します。ガチャのプレイヤーに名簿を紐づけてから抽選してください",
                            "連携中は会場では受付QR（配布タブに表示）だけを案内し、個別の受取URLを手渡しする必要はありません",
                            "初来などで受取人が二重になった場合は、だんごシェアリンク管理画面でマージできます（常連は名簿紐づけで防げます）",
                            "プレイヤー行の配布アイコンから、受取人名簿との紐づけ・配布の再同期・管理画面への移動ができます",
                            "プレイヤー履歴画面（〇〇の履歴）右上のリンクアイコンからも、同じ配布・リンク集の操作ができます",
                            "だんごシェアリンク連携は「配布」タブの「連携を開始する」でこの端末への許可が必要です（だんごシェアリンク側のツール連携だけでは足りません）",
                            "配布タブで景品ごとにだんごシェアリンクのファイルを紐づけてから抽選してください。プルダウンで既存ファイルを選ぶか、行の右のアップロードから新規登録できます",
                            "品目はだんごで作った場合は「名前で自動マッチ」「ファイルの一括登録」が使えます。だんごシェアリンク側でファイルを載せた場合は「アセットを再取得」してから選べます",
                            "だんごシェアリンクで品目ごと作り直す場合は、配布タブの「その他」から「だんごシェアリンクから構成を取り込む」を使えます（だんごの品目が上書きされます）",
                            ...
                            "配布タブの「その他」→「接続を解除」で、この端末の接続情報を削除できます（だんごシェアリンク側の設定 → 外部連携で連携を解除すると完全に切断されます）",
                            "だんごシェアリンクでキャンペーンの「ツール連携を一時停止」にしている間は、だんごツールからの配布更新はできません（管理画面で再開してください）",
                            "配布枠が見つからないとき、プレイヤーの配布画面から管理画面を開くと、該当する受取人の位置へ移動します",
                            "データ連携で設定を別端末に移したあとは、ガチャの配布タブからだんごシェアリンクへの再接続が必要です",
                            "だんごシェアリンクで再度連携を許可すると、同じ用途の古い接続は自動で無効になり、新しい1件だけが使われます",
                    ...
                    },
                    {
                        ...
                        items: [
                            ...
                            "「結果を画像で共有」ボタンで、現在の結果をテンプレートに合わせた画像として保存し、X投稿画面を開けます。返信先を設定している場合はX返信画面を優先します",
                            "プレイヤーの履歴画面では、このガチャの累計一覧と、各回ごとの結果をタブで切り替えて確認・共有できます",
                            "各回の結果カードにも「結果を画像で共有」ボタンがあり、その回の結果だけをテンプレート画像として保存・共有できます（返信先設定時は返信画面を優先）",
                ...
                sections: [
                    ...
                    },
                    {
                        ...
                        items: [
                            ...
                            "ヘッダーの「画像で共有」ボタンで、現在の出目（プレイヤー名・図柄並び・役）を1枚の画像にし、ダウンロードやXの投稿画面を開けます。返信先を設定していない場合は対応端末で共有シートを使って画像付き共有ができます。返信先を設定している場合は返信画面を優先します",
                        ]
                    }
                ]
            };
            ...
            return {
                ...
                sections: [
                    ...
                    },
                    {
                        title: "結果と共有",
                        items: [
                            "「画像で共有」ボタン（ヘルプの左）で進捗を1枚の画像にし、返信先を設定していない場合は対応端末で共有シートの画像付き共有、返信先を設定している場合はダウンロード後にX返信画面を開けます",
                        ]
                    },
                    {
                        title: "OBSブラウザソース",
                        items: [
                            "設定（歯車）の「OBS用URLをコピー」で、透過表示用のURL（/counter?obs=1）をコピーできます",
                            "OBSのブラウザソースにそのURLを貼り付けると、メニューや背景を隠した表示になります。項目のタップでカウントできます",
                            "テンプレートの切替や項目の追加は通常の /counter で行ってから、OBS用URLを開いてください",
                            "OBS側で背景色を透過にする設定（クロマキー等）が必要な場合があります",
                        ]
                    }
                ]
            };
