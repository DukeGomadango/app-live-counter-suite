...
export default function PlayerLinkCollectionModal({
    ...
    const handleCopyUrl = () => {
        const reception = integrationConfig?.campaignReceptionUrl?.trim();
        if (reception) {
            navigator.clipboard.writeText(reception);
            showToast("受付URLをコピーしました", "success");
            return;
        }
                ...
                <div className="p-4 flex flex-col gap-4 max-h-[70vh] overflow-y-auto">
                    {isIntegrationActive && integrationConfig ? (
                        <div className="flex flex-col gap-4">
                            ...
                            >
                                <strong>推奨運用:</strong> プレイヤーに<strong>受取人名簿</strong>を紐づけ、会場では<strong>受付QR</strong>（配布タブ）を案内してください。枠が二重になった場合のみ管理画面で<strong>マージ</strong>します。
                            </div>
